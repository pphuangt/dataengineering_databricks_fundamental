...
-- MAGIC %md
-- MAGIC # 7 Bonus Lab - AUTO CDC INTO with SCD Type 1
-- MAGIC
...
-- MAGIC
-- MAGIC ### REQUIREMENTS:
-- MAGIC It’s your job to complete the Spark Declarative Pipeline by adding the `AUTO CDC` statement to perform SCD Type 1.
-- MAGIC
...
-- MAGIC
-- MAGIC **NOTE:** The run the solution for this lab go to step **F. Lab Solution (OPTIONAL)**.
...
-- MAGIC
-- MAGIC    e. In the new tab you should see the folder: **cdc_type_1_pipeline**. 
-- MAGIC
-- MAGIC    f. Open the **cdc_type_1_pipeline** folder and select the **cdc_employees.sql** notebook.

-- COMMAND ----------

-- MAGIC %md
-- MAGIC #### TO DO: Review the code in the cdc_employees.sql file and complete the `AUTO CDC INTO` statement to perform SCD Type 1.
...
-- MAGIC
-- MAGIC [AUTO CDC INTO (Lakeflow Spark Declarative Pipelines)](https://docs.databricks.com/gcp/en/dlt-ref/dlt-sql-ref-apply-changes-into)
...
-- MAGIC
-- MAGIC ![Lab 7 Pipeline Run](./Includes/images/lab_7_pipelinerun.png)
...
-- MAGIC
-- MAGIC Each table in this solution pipeline will end with **_solution** and the code cells below will need to be modified.
...
-- MAGIC
-- MAGIC ![Lab 7 Challenge Run](./Includes/images/lab_7_challengesolution.png)

...
-- 2. In your pipeline project, you should see three folders: an 'exploration' folder, 'orders' folder, and ‘python_excluded’ folder.
  ...
  -- c. The 'python_excluded' folder contains the Python verison of the SQL code in this file.
...
-- 4. Complete the following steps to explore and modify the settings of the pipeline:
  ...
  -- b. The first section provides information about your pipeline, such as the Pipeline ID, type, name,mode, creator, owner and run as. You can select the 'edit' icon to modify any of the settings. Leave as is.
  ...
  -- d. The 'Default location for data assets' section specifies the default catalog and schema to use in the pipeline.
    ...
    -- **TODO**: Select 'Edit the catalog and schema':
        -- Set the default catalog to your 'workspace' catalog
        -- Set the default schema to 'default'.
        -- Click 'Save'.
        -- NOTE: With Lakeflow Spark Declarative Pipelines you can publish streaming tables and materialized views to any catalog and schema, you are not restricted to the defaults.
  ...
  -- f. The 'Configuration' section enables you to add variables with default values to use in your pipeline code. Complete the following steps to add the 'source' variable pointing to your raw data volume:
      ...
      -- **TODO**: c. For 'Value', paste the volume path of your `/Volumes/pipeline/pipeline_data/data` volume. This is the volume with our JSON files to ingest that you copied earlier.
          -- EXAMPLE: `/Volumes/pipeline/pipeline_data/data`

      -- **TODO**: d. Select 'Save'

  -- g. The 'Usage' section enables you to add a Usage policy for Serverless compute. Budget policies consist of tags that are applied to any serverless compute activity incurred by a user assigned to the policy. The tags are logged in your billing records, allowing you to attribute serverless usage to specific budgets.

  -- h. Expand the 'Advanced Settings' section and select 'Legacy pipeline settings'. In Advanced Settings, you can modify the following:

      -- a. 'Channel' - Controls the pipeline runtime version.
      
      -- b. 'Event Logs' - The pipeline event log contains all information related to a pipeline, including audit logs, data quality checks, pipeline progress, and data lineage. You can write this as a table to the schema. Leave this deselected for now.
...
-- SQL Pipeline Code
-------------------------------------------------------
-- NOTE: The default catalog is set to your 'workspace' catalog, so specifying the catalog is not required for the code below since we are using the default catalog.
-------------------------------------------------------
...
-- 6. Complete the following steps to verify the correctness of the pipeline source code without updating any tables:
    ...
    -- TROUBLESHOOTING: If your pipeline returns an error, ensure the following are set correctly:
      -- The default catalog is set to your 'workspace' catalog.
      -- The default schema is 'default'.
      -- The 'source' configuration variable references your volume path: '/Volumes/pipeline/pipeline_data/data'.
...
-- 7. Once ready, run the pipeline by selecting the drop-down next to the 'Run pipeline' button in the top navigation bar. This provides two options:
    ...
    -- a. 'Run pipeline with full table refresh': This streams data through each table and computes results in the materialized view. Re-running it won’t reprocess files that were already ingested.

    -- b. WARNING!!!! 'Run now with Different Settings': This reprocesses **ALL** data. Use with caution—if your source files are deleted periodically, a full refresh will remove all data and fail to re-read deleted files.
...
-- 9. After the pipeline completes, explore it using the new editor:
  ...
  -- **TODO**: g. Use the navigation bar in the bottom window to explore the 'Data', 'Columns', 'Table metrics', and 'Performance' for the materialized view.


-- 10. You can also select objects directly within the DAG to view related information on the right. Select the 'orders_silver_demo2' object to update the details in the bottom window.

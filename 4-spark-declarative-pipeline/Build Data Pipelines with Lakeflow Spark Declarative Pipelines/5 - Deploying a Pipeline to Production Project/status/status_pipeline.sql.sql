...
-- MAGIC
-- MAGIC In this pipeline, the **status** pipeline is implemented within a notebook. Lakeflow Spark Declarative Pipelines support using `.py`, `.sql`, or notebook files as pipeline sources.
-- MAGIC
...
-- MAGIC ![Pipeline](../../Includes/images/demo5_pipeline_image_run1.png)
...
-- MAGIC One way to join two streaming tables in Spark Declarative Pipelines is by creating a materialized view that performs the join.  This approach takes all rows from each streaming table and executes a full inner join operation and incorporates optimizations where applicable.
-- MAGIC
-- MAGIC **NOTES:**
-- MAGIC
...
-- MAGIC
-- MAGIC       - **REQUIRED FOR THE EVENT LOG:** In the **Event logs** section:
-- MAGIC         - Select **Publish event log to Unity Catalog**.
...
-- MAGIC
-- MAGIC     ![Pipeline Demo 5](../../Includes/images/demo5_pipeline_image_run1.png) 
-- MAGIC
...
-- MAGIC %md
-- MAGIC #### REQUIRED: In the main notebook complete the steps in step **D. Land More Data to Your Data Source Volume**.
...
-- MAGIC
-- MAGIC    f. The **orders_by_date_gold_demo5** materialized view contains 11 rows.
-- MAGIC
-- MAGIC     ![Pipeline Demo 5](../../Includes/images/demo5_pipeline_image_run2.png) 
-- MAGIC

-- COMMAND ----------

-- MAGIC %md
-- MAGIC 6. In the window at the bottom, select the **Expectations** link for the **status_silver_demo5** table. It should contain the value **1 met | 1 unmet**. Notice that in this run, 7.6% (31 rows) for the **valid_order_status** expectation returned a warning.
-- MAGIC
